...
Obsidian 用户可以把 wiki/ 下的 md 笔记转为精美 HTML 作为「知识消费层」——HTML 在浏览器中的阅读体验远超 Markdown 预览。详见 `references/obsidian-workflow.md`（含模板选择建议、demo 记录、批量转换脚本）。

核心约定：HTML 与 md 同目录，不放 raw/。article 模板是默认推荐。

## 能力2扩展：Obsidian wiki/ 批量 → HTML

当用户说「把知识库批量转成HTML」「wiki全部转网页」时，走这个流程。

### 命名约定：`00-` 前缀

Obsidian 文件列表按文件名排序。HTML 文件名加 `00-` 前缀后，**永远排在同目录最前面**，用户一眼就能看到。md 源文件紧随其后。

```
wiki/AI/
  00-AI Agent系统设计模式.html   ← HTML 排第一
  AI Agent系统设计模式.md         ← md 源文件
  00-构建高效Agent.html
  构建高效Agent.md
```
...
脚本路径：`scripts/batch_wiki_to_html.sh`（也部署到 Obsidian vault 的 `scripts/` 目录）

```bash
# 找所有 wiki/ 下的 md，排除 meta 文件和已转 HTML
# 跳过 <200 字节的空文件
# 输出到同目录，00- 前缀
bash scripts/batch_wiki_to_html.sh
```

脚本逻辑：
1. `find wiki/ -name '*.md'` 排除 `index.md`、`log.md`、`overview.md`、`README.md`、`entities/`、`sources/`、`topics/`、`00-*`
2. 跳过 <200 字节的文件（空壳/占位符）
3. 调用 `md_to_html.py --theme article` 转换
4. 输出到同目录 `00-{basename}.html`
5. 已存在且比 md 新的 HTML 自动跳过（增量转换）

### YAML frontmatter 冲突（已知坑）

Obsidian 笔记中 `---` 可能出现在正文中间（如分隔线），Pandoc 会误认为第二个 YAML delimiter 导致解析失败。

**症状**：`Error parsing YAML metadata at (line N, column 1)`，N 通常是正文中间的 `---` 行号。

**修复**：对这类文件用 pandoc 直接转换，禁用 YAML 解析：

```bash
pandoc input.md \
  -f markdown-yaml_metadata_block \
  --template=templates/article/template.html5 \
  --metadata title="页面标题" \
  -o 00-output.html
```

模板路径：`SKILL_ROOT / "templates" / {theme} / "template.html5"`

### 已转换文件清单

首次批量转换完成于 2026-06-01，共 93 个 HTML 文件，覆盖 wiki/ 下 AI(46)、临床药学(28)、学习方法(7)、计算机基础(3)、效率工具(3) 等子目录。脚本存于 Obsidian vault 的 `scripts/batch_wiki_to_html.sh`。
...
当用户说「把 wiki/ 下所有笔记都转成 HTML」或「批量转」时，使用批量转换工作流。

### 已知坑

1. **YAML `---` 冲突**：Markdown 正文中的 `---`（水平线）会被 Pandoc 误判为 YAML frontmatter 结束符，导致 `Error parsing YAML metadata`。修复：用 `-f markdown-yaml_metadata_block` 禁用 YAML 解析，或用 `--metadata title="..."` 手动传标题。
2. **小文件跳过**：< 200 字节的 md 通常是空占位符，转换无意义。
3. **Pandoc 模板路径**：md_to_html.py 的模板在 `SKILL_ROOT/templates/<theme>/template.html5`，直接调 pandoc 时需要手动指定。

### 批量转换脚本模板

```bash
#!/bin/bash
# 批量 md → HTML，每个笔记独立文件夹（md + html 放一起）
WIKI_DIR="/path/to/wiki"
SCRIPT="/path/to/md_to_html.py"
THEME="article"

find "$WIKI_DIR" -name '*.md' -type f \
  ! -name 'index.md' ! -name 'log.md' ! -name 'README.md' \
  ! -path '*/entities/*' ! -path '*/sources/*' ! -path '*/topics/*' \
  ...
  # 跳过已存在的文件夹
  [ -d "$folder/$base.html" ] && continue
  
  mkdir -p "$folder"
  mv "$md_file" "$folder/$base.md"
  python3 "$SCRIPT" "$folder/$base.md" --theme "$THEME" -o "$folder/$base.html" 2>&1
done
```

### 用户偏好：文件夹-per-笔记

用户偏好每个笔记一个文件夹，md + html 放在一起，方便一眼看出源文件：
```
wiki/AI/AI Agent系统设计模式/
  AI Agent系统设计模式.md    ← 源文件
  AI Agent系统设计模式.html  ← 阅读版
```
不要用 `00-` 前缀方式（html 和 md 平铺在同一目录）。
...
| Obsidian wiki/ 批量转换脚本 | `scripts/batch_wiki_to_html.sh` |
